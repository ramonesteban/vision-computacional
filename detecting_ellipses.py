...
class Ellipses:
    ...
    def draw_some_ellipses(self, boxsize, image):
        newdraw = ImageDraw.Draw(image)
        max_w, max_h = get_image_size(image)

        for dim in boxsize:
            x = random.randint(50, max_w)
            y = random.randint(50, max_h)
            xdim, ydim = dim
    ...
    def draw_ellipses_found(self, image, ellipses_found):
        ...
        for ellipse in ellipses_found:
            ...
            draw.text((x+4, y-4), str(counter), fill=(255, 255, 255))
            counter += 1
        return image

    def action(self):
        original_image = Image.new('RGB', (SIZE, SIZE), (255, 255, 255))
        original_image = self.draw_some_ellipses([(60, 30), (40, 70), (20, 40), (20, 20)], original_image)
        ...
        image = self.draw_ellipses_found(image_bfs, ellipses_found)
